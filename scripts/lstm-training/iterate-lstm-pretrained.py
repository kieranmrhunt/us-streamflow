...
#start_date = dt(2020,10,21)


dates = [dt(2020,11,1)+td(days=i) for i in range(24)]
...
for start_date in dates:
	...
	f = open(start_date.strftime(compute_dir+'operational_output/lstm_%Y%m%dT%H.csv'), 'w+')
	...
	for station_code in station_codes:
		...
		df = pd.read_csv(compute_dir+"operational-catchment-data/{}{}.csv".format(station_code, f_dstring), 					 )
		...
		if np.abs(td_min)<=td(hours=24):
			gt_vals = gt.Value.values[itg-16:itg-8]
		if 'Ice' in gt_vals:
			delta = 0#ens_mean[it]
		else:
			en_val = np.mean(ens_mean[ite-8:ite-4])
		...
		#plt.plot(ens_dates,ens_flows, color='grey', lw=.5)
		#plt.plot(ens_dates,ens_mean, color='r')
		#plt.plot(ens_dates,average[-len(ens_dates):], color='k')
		#plt.show()
