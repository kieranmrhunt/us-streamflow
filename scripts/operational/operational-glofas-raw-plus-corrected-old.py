...
for start_date in [dt(2021,3,1)+td(days=i) for i in range(26)]:
	...
	f_raw = open(start_date.strftime(compute_dir+'test-output/raw_%Y%m%dT%H.csv'), 'w+')
	f_bc = open(start_date.strftime(compute_dir+'test-output/bcmatrix_%Y%m%dT%H.csv'), 'w+')
